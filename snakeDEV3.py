"""Current Dev: use delchar instead of rewriting everythong.
...
if importSuccess == True:
  ...
  chomp.set_volume(.8)
  money = pygame.mixer.Sound('bloopbloop.wav')
  money2 = pygame.mixer.Sound('money.wav')
  life = pygame.mixer.Sound('life.wav')
  block = pygame.mixer.Sound('block.wav')
  gameover = pygame.mixer.Sound('gameOver.wav')
  levelwin = pygame.mixer.Sound('levelwin.wav')
  randomnoise = pygame.mixer.Sound('randomnoise.wav')
  spooky = pygame.mixer.Sound('spooky.wav')

###myList holds [y, x] values
soundList = [chomp, money, life, block, gameover, levelwin, randomnoise, spooky, money2]
  ...
  while 1:
    ...
    if q == ord('w'):
      ...
      if currentPos == 0:
        ...
        pass
        num = random.choice(range(len(soundList)))
        soundList[num].play()
        for i in soundList:
...
def takeCode(points, applePoints, moneyPoints, level, mortal, lives, mpMOD, apMOD):
  ...
  while temp < 1:
    ...
    while 1:
      screen.clear()
      screen.addstr(dims[0] / 2 - 1, dims[1] / 2 - 5, 'ENTER CODE', curses.color_pair(5))
      screen.addstr(dims[0] / 2 + 1, dims[1] / 2 - len(CODE)/2, CODE + '  ', curses.color_pair(3))
      screen.move(dims[0] - 1, dims[1] - 1)
      q = screen.getch()
      if q == 10:
        break
    ...
    if CODE == 'BBDD':
      ...
      level = 9
      cheat = True
    elif CODE == 'SHOT': # LEVEL 11
      level = 10
      cheat = True
    elif CODE == 'SKLL': # LEVEL 12
      level = 11
      cheat = True
    elif CODE == 'BOOM': # LEVEL 13
      level = 12
      cheat = True
    elif CODE == 'BOXX': # LEVEL 14
      level = 13
      cheat = True
    elif CODE == 'DUFF': # LEVEL 15
      level = 14
      cheat = True       
    elif CODE == 'GLOB': # LEVEL 16
      level = 15
      cheat = True
...
def selectDifficulty():
  screen.clear()
  screen.addstr(dims[0] / 2 - 3, dims [1] / 2 - 5, 'DIFFICULTY', curses.color_pair(3))
  currentPos = 2
  difficultyList = ['NOVICE', 'EASY', 'NORMAL', 'HARD', 'INSAAAAANE!!!']
  colorList = [curses.color_pair(5), curses.color_pair(5), curses.color_pair(3), 
  curses.color_pair(5), curses.color_pair(5)]
  
  while 1:
    screen.addstr(dims[0] / 2 - 2, dims[1] / 2 - len(difficultyList[0]) / 2, difficultyList[0], colorList[0])
    screen.addstr(dims[0] / 2 - 1, dims[1] / 2 - len(difficultyList[1]) / 2, difficultyList[1], colorList[1])
    screen.addstr(dims[0] / 2, dims[1] / 2 - len(difficultyList[2]) / 2, difficultyList[2], colorList[2])
    screen.addstr(dims[0] / 2 + 1, dims[1] / 2 - len(difficultyList[3]) / 2, difficultyList[3], colorList[3])
    screen.addstr(dims[0] / 2 + 2, dims[1] / 2 - len(difficultyList[4]) / 2, difficultyList[4], colorList[4])
    screen.move(dims[0] - 1, dims[1] - 1)
    screen.refresh()
    q = screen.getch()
    if q == ord('w') or q == ord('a'):
      currentPos += 4
    elif q == ord('s') or q == ord('d'):
      currentPos += 1
    elif q == 10:
      break
    currentPos %= 5
    for i in range(len(colorList)):
          colorList[i] = curses.color_pair(5)
    
    colorList[currentPos] = curses.color_pair(3)
  return currentPos
...
def highScore():
  ...
  while 1:
    ...
    if temp == '':
      ...
      temp = temp.split('=')
    highScoreList[0].append(temp[0])
    highScoreList[1].append(temp[1])
  f.close()
  i = 0
  while i <= 9:
    
    if points > int(highScoreList[1][i]):
      NAME = ''
      while 1:
        
        screen.clear()
        screen.addstr(dims[0]/2 - 3, dims[1] / 2 - 9, 'NEW HIGH SCORE!!!!', curses.color_pair(5))
        screen.addstr(dims[0]/2 - 1, dims[1]/2 - 5, 'ENTER NAME:', curses.color_pair(5))
        screen.addstr(dims[0]/2 + 1, dims[1]/2 - len(NAME)/2, NAME, curses.color_pair(3))
        screen.refresh()
        q = screen.getch()
        if q == 10:
          if len(NAME) > 0:
            highScoreList[0].insert(i, NAME)
            break
          else:
            screen.beep()
        elif q == 127:
          ...
          NAME = NAME + chr(q)
      highScoreList[1].insert(i, str(points))
      del highScoreList[0][-1]
      del highScoreList[1][-1]
      break
    i += 1
  screen.clear()
  for i in range(len(highScoreList[0])):
    screen.addstr(dims[0]/2 - 5 + i, dims[1]/2 + 3, highScoreList[1][i], curses.color_pair(3))
    screen.addstr(dims[0]/2 - 5 + i, dims[1]/2 - 6, highScoreList[0][i], curses.color_pair(3))
  screen.move(dims[0] - 1, dims[1] - 1)
  screen.refresh()
  f = open('highscores.txt', 'w')
  for i in range(len(highScoreList[0])):
    f.write('%s=%s' %(highScoreList[0][i], str(highScoreList[1][i]) ) )
  f.close()
  time.sleep(2)
  screen.getch()     
...
difficultyInt = selectDifficulty()

if difficultyInt == 0:
  difficulty = 1.75
elif difficultyInt == 1:
  difficulty = 1.3
elif difficultyInt == 2:
  difficulty = 1.0
elif difficultyInt == 4:
  difficulty = .75
else:
  difficulty = .3
